...
from src.models import Conversation, Message, Action
...
async def get_analytics(
    ...
    escalated_result = await session.execute(
        ...
        .where(and_(*filters, Conversation.status == "escalated"))
    )
    ...
    resolved_result = await session.execute(
        select(func.count(Conversation.id))
        .where(and_(*filters, Conversation.status == "resolved"))
    )
    ...
    csat_result = await session.execute(
        select(
            func.avg(Conversation.csat_score),
            func.count(Conversation.csat_score)
        )
        .where(and_(*filters, Conversation.csat_score.isnot(None)))
    )
    ...
    actions_taken = {a: c for a, c in action_rows}
...
async def get_analytics_summary(
    ...
    result = await session.execute(
        select(
            ...
            func.sum(
                func.cast(Conversation.status == "escalated", Integer)
            ),
        )
        .where(and_(*filters))
    )
