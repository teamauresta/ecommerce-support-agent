...
class ShopifyClient:
    ...
    async def get_order_by_number(self, order_number: str) -> dict[str, Any] | None:
        ...
        data = await self._request("GET", "orders.json", params={"name": number, "status": "any"})
    ...
    async def add_order_note(self, order_id: str, note: str) -> dict[str, Any]:
        ...
        data = await self._request("PUT", f"orders/{order_id}.json", json={"order": {"note": note}})
        return data.get("order", {})
    ...
    async def get_customer_by_email(self, email: str) -> dict[str, Any] | None:
        ...
        data = await self._request(
            "GET", "customers/search.json", params={"query": f"email:{email}"}
        )
