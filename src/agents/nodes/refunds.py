...
async def handle_refunds(state: ConversationState) -> dict[str, Any]:
    ...
    decision = await _make_refund_decision(
        order_data, refund_amount, message, state
    )
    ...
    if decision.get("auto_approve"):
        ...
        if refund_result["success"]:
            ...
            updates["agent_reasoning"] = f"Refund of ${decision['amount']} auto-approved and processed"
        else:
            updates["response_draft"] = (
            ...
            updates["agent_reasoning"] = f"Refund approved but processing failed: {refund_result.get('error')}"
        ...
        # Not eligible
        updates["response_draft"] = _generate_refund_denied_response(
            order_data, decision, state
        )
...
def _generate_refund_approved_response(
    ...
    if decision.get("requires_return"):
        response += (
            "Since this is a product issue, no return is needed — please keep or donate the item.\n\n"
        )
